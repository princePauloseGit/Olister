...
page 50201 "Activity Card"
{
                            ...
                            if ActivityType.Get(Rec."Activity type") then begin
                                case ActivityType."Default Next Step" of
                                    ...
                                    "Activity Next Step"::"Create Purchase Price List":
                                        begin
                                            if not PurchListVisible then
                                                Error('You do not have access to create Purchase Price Lists. Refer to user setup. Please contact your system administrator.');
                                            CanCreateSPL := false;
                                            CanCreatePPL := true;
                                    ...
                                    "Activity Next Step"::"Create Sales Price List":
                                        begin
                                            if not SalesListVisible then
                                                Error('You do not have access to create Sales Price Lists. Refer to user setup. Please contact your system administrator.');
                                            CanCreatePPL := false;
                                            CanCreateSPL := true;
                                            "Price List".Getdata(Rec, CanCreatePPL, CanCreateSPL);
                                            if "Price List".RunModal() = Action::OK then begin
            ...
            action(ShareActivity)
            {
                ...
                begin
                    if UserSetup.Get(UserId) then begin
                        if UserSetup."Has Activity Sharing Rights" then
                            Controller.SendEmail(Rec)
                        else
                            Error('You do not have rights to share Activities. Refer to user setup. Please contact your system administrator.');
                    end;
    ...
    begin
        ...
        if Rec."Activity type" <> '' then begin
            ...
            if ActivityType."Default Next Step" = "Activity Next Step"::"Create Sales Price List" then begin
                CanCreateSPL := true;
            end
            else begin
                CanCreateSPL := false;
            ...
            if ActivityType."Default Next Step" = "Activity Next Step"::"Create Purchase Price List" then begin
                CanCreatePPL := true
            end
            else begin
                CanCreatePPL := false;
        ...
        if "User Setup".Get(UserId) then begin
            if "User Setup"."Project Purch. List access" = true then begin
                ...
                AttachFctboxVisible := true;
            end
            else begin
                PurchListVisible := false;
                AttachFctboxVisible := false;
            end;
        ...
        If ActivityType."Default Next Step" = ActivityType."Default Next Step"::" " then
            AttachFctboxVisible := true;
